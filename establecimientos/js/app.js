...
// jala los datos para mostrar y modificar
		$('#dataUpdate').on('show.bs.modal', function (event) {
		  ...
		  var id_categoria= button.data('id_categoria') // Extraer la información de atributos de datos
 ...
 (function($) {
  ...
  $(document).ready(function(){
    $('#id_gerencia').on('change', function(){
        if($('#id_gerencia').val() == ""){
          $('#id_oficina').empty();
          $('<option value = "">Selecciona una Oficina</option>').appendTo('#id_oficina');
          $('#id_oficina').attr('disabled', 'disabled');
        }else{
          $('#id_oficina').removeAttr('disabled', 'disabled');
          $('#id_oficina').load('selecciona_oficina.php?id_gerencia=' + $('#id_gerencia').val());
        }
    });
        $('#id_gerencia_').on('change', function(){
        if($('#id_gerencia_').val() == ""){
          $('#id_oficina_').empty();
          $('<option value = "">Selecciona una Oficina</option>').appendTo('#id_oficina_');
          $('#id_oficina_').attr('disabled', 'disabled');
        }else{
          $('#id_oficina_').removeAttr('disabled', 'disabled');
          $('#id_oficina_').load('selecciona_oficina.php?id_gerencia=' + $('#id_gerencia_').val());
        }
    });
  });
