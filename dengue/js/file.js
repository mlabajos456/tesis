	function load(page,id){
		var parametros = {"action":"ajax","page":page,"id_articulo":id};
		$("#loader").fadeIn('slow');
		$.ajax({
			url:'lista_l.php',
			data: parametros,
...
$('#datafile').on('show.bs.modal', function (event) {
		  ...
		  var id= button.data('id_articulo') // Extraer la información de atributos de datos
		
		  var modal = $(this)
		  $("#guardarfile")[0].reset();
		  modal.find('.modal-body #id_articulo').val(id)
...
$( "#guardarfile" ).submit(function( event ) {
	if($("#nom_legajo").val()==""||$("#id_articulo").val()==""){
