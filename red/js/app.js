    ...
    success: function (datos) {
      if (datos == 1) {
        showToastPosition(
          "top-right",
          "Error!",
          "Hubo un problema al registrar la red",
          "warning"
        );
...
$("#dataUpdate").on("show.bs.modal", function (event) {
  ...
  var id_red = button.data("id_red"); // Extraer la información de atributos de datos
  var desc_red = button.data("desc_red"); // Extraer la información de atributos de datos
  ...
  modal.find(".modal-body #id_red2").val(id_red);
  modal.find(".modal-body #desc_red2").val(desc_red);
});
