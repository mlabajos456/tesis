	function load(page,id_usuario){
		var parametros = {"action":"ajax","page":page,"id_usuario":id_usuario};
		$("#loader").fadeIn('slow');
		$.ajax({
			url:'lista.php',
			data: parametros,
			 beforeSend: function(objeto){
			$("#loader").html("<img src='loader.gif'>");
			},
			success:function(data){
				$(".outer_div").html(data).fadeIn('slow');
				$("#loader").html("");
			}
		})
	}
// procede a guardar los recien agregados nuevos

$( "#guardarDatos" ).submit(function( event ) {
		var parametros = $(this).serialize();
			 $.ajax({
					type: "POST",
					url: "modelos/agregar.php",
					data: parametros,
					 beforeSend: function(objeto){
						$("#datos_ajax_register").html("<center><img src='../../images/load.gif' height='100' width='100'></center>");
						$("#btsubmit").attr("disabled", true);
					  },
					success: function(datos){
					$("#datos_ajax_register").html(datos);
					$("#guardarDatos")[0].reset();
		            $("#btsubmit").attr("disabled", false);
					$('#dataRegister').modal('hide');
		            $('.modal-backdrop').hide();
		            showSwal('success-message');
		            var id_usuario=$("#iddle").val();
					load(1,id_usuario);

				  }
			});
		   event.preventDefault();

		});


// jala los datos para mostrar y modificar
		$('#dataUpdate').on('show.bs.modal', function (event) {

		  var button = $(event.relatedTarget) // Botón que activó el modal
		  var id_usuario= button.data('id_usuario') // Extraer la información de atributos de datos
		  var nom_usuario = button.data('nom_usuario') // Extraer la información de atributos de datos
		  var nom_empleado = button.data('nom_empleado')
		  var estado_usuario = button.data('estado_usuario')
		  var modal = $(this)
		  $("#actualidarDatos")[0].reset();
		  //modal.find('.modal-title').text('Modificar Departamento: '+nom_ciudad)
		  modal.find('.modal-body #id_usuario').val(id_usuario)
		  modal.find('.modal-body #nom_usuario').val(nom_usuario)
		  modal.find('.modal-body #nom_empleado').val(nom_empleado)
		  modal.find('.modal-body #estado_usuario').val(estado_usuario)
		})


//Modificar los datos recientemente jalados
$( "#actualidarDatos" ).submit(function( event ) {
		var parametros = $(this).serialize();
			 $.ajax({
					type: "POST",
					url: "modelos/modificar.php",
					data: parametros,
					 beforeSend: function(objeto){
						$("#datos_ajax_register").html("<center><img src='../../images/load.gif' height='100' width='100'></center>");
					  },
					success: function(datos){
					$("#datos_ajax").html(datos);
					
					$("#dataUpdate").modal("hide");
					$('.modal-backdrop').hide();
					
					load(1);
				  }
			});

		  event.preventDefault();

		});

//eliminar

      (function($) {
      showSwal = function(type,id){  
        'use strict';
        if(type === 'swalEliminar'){
            swal({
              title: 'Está seguro de eliminar el registro?',
              text: "El registro se eliminará permanentemente!",
              type: 'warning',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              confirmButtonText: 'Si'
            }).then(function () {
            	//alert(id);
              $.ajax({
                type: "POST",
                url: "modelos/eliminar.php",
                data: 'id='+id,
                 beforeSend: function(objeto){
                 $("#datos_ajax_register").html("<center><img src='../../images/load.gif' height='100' width='100'></center>");
                  },
                success: function(datos){
                $(".datos_ajax_register").html(datos);
                var id_usuario=$("#iddle").val();
                load(1,id_usuario);

                  $('#dataDelete').modal('hide');
                  $('.modal-backdrop').hide();

                       }

               });
          })

      }else if(type === 'success-message'){
        	swal(
              'Tarea Exitosa!',
              'Datos registrados!',
              'success'
          )

    	}
    }
 })(jQuery);

  $(document).ready(function(){
    $('#id_modulo').on('change', function(){
        if($('#id_modulo').val() == ""){
          $('#id_sub_modulo').empty();
          $('<option value = "">Selecciona un sub módulo</option>').appendTo('#id_sub_modulo');
          $('#id_sub_modulo').attr('disabled', 'disabled');
        }else{
          $('#id_sub_modulo').removeAttr('disabled', 'disabled');
          $('#id_sub_modulo').load('selecciona_sub_modulo.php?id_modulo=' + $('#id_modulo').val());
        }
    });
     
  });